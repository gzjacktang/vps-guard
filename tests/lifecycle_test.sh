...
test_missing_rollback_state_blocks_uninstall_fail_closed() {
  ...
  assert_status 3
  assert_output_contains "状态异常"
  [[ -d "$TEST_ROOT/program" && -x "$TEST_ROOT/vps-guard" ]]
}

test_symlink_rollback_state_blocks_uninstall_fail_closed() {
  setup_lifecycle_test
  trap teardown_lifecycle_test RETURN
...
test_pending_ssh_enrollment_cleanup_blocks_uninstall() {
  setup_lifecycle_test
  trap teardown_lifecycle_test RETURN
