...
- 标准快速流程只询问 SSH 新端口与业务端口；SSH、防火墙、Fail2ban 作为一个完整快照、一个计时器和一次确认共同提交。
- 使用版本化程序目录和固定 `vps-guard` launcher；手动更新检查不下载或执行远程代码，更新前备份现有程序。
- 默认卸载只移除程序并保留系统配置、快照和日志；数据清理需要独立二次确认。
...
去掉 `--dry-run` 即可真实执行。高风险操作默认配置 5 分钟自动回滚——如果操作导致断连，等待计时结束后旧配置会自动恢复。成功登录新端口后执行命令输出的 `wizard confirm` 即完成提交。
