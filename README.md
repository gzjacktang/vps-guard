...
VPS Guard 是一个面向 Debian 与 Ubuntu VPS 的中文安全管理工具。项目将统一管理 SSH、nftables、Fail2ban、配置快照和自动回滚；当前已提供只读系统诊断、网络环境冲突预检、SSH 端口两阶段迁移、SSH 密钥与可选认证加固、nftables 安全基线、基础端口管理、快照恢复与 systemd 定时回滚，Fail2ban 管理仍在后续切片中。
...
- 防火墙只管理 `table inet vps_guard`；SSH 端口与认证加固只管理标准 sshd 配置范围、自有 drop-in 及经用户明确选择的目标 `authorized_keys`。Fail2ban 尚未交付。
