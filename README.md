...
VPS Guard 是一个面向 Debian 与 Ubuntu VPS 的中文安全管理工具。项目将统一管理 SSH、nftables、Fail2ban、配置快照和自动回滚；当前已提供只读系统诊断、网络环境冲突预检、nftables 安全基线、基础端口管理、快照恢复与 systemd 定时回滚，SSH 写入与 Fail2ban 管理仍在后续切片中。
...
- `status` 与 `preflight` 始终只读；只有明确的 `firewall` 写入子命令会修改自有 nftables 范围。
- 防火墙写入功能只管理 `table inet vps_guard`；SSH 与 Fail2ban 写入功能尚未交付。
