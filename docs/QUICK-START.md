...
密钥生成、禁用密码、root 登录策略、来源 IP/CIDR、接口和出站限制不在快速问题中；它们仍属于 SSH、防火墙或 Fail2ban 子菜单。进入子菜单再返回时，向导保留已经填写的方案和端口，不要求从头输入。高级子菜单的写入使用其自身独立事务；必须先在新会话完成对应确认，向导才允许继续。之后的快速应用仍只创建一个涵盖当时完整起点的快照和一个回滚任务。
...
若保持 SSH 不变，也应先验证业务端口及 Fail2ban 状态，再显式执行 `wizard confirm`。未确认或连接异常时等待自动回滚；不要删除状态文件或停止 systemd 任务。
