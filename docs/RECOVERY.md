...
程序更新前会把当前版本复制到 `/usr/local/lib/vps-guard/program-backups/<UTC>-<旧版本>`，并保留 `/usr/local/lib/vps-guard/releases/<旧版本>`。若新版本入口异常，从带外控制台把 `current` 恢复为已知旧版本并运行 `vps-guard version`；不要删除旧版本目录中仍可能被 systemd 回滚任务引用的脚本。

程序代码回退不改变 `/etc` 安全配置。配置异常仍应使用本页的快照或模块恢复流程。存在非终态自动回滚时，正常卸载会返回退出码 3；不要用手工 `rm` 绕过。
...
防火墙变更不会自动创建回滚。若变更后无法建立新 SSH 会话，请使用云控制台、串行控制台或救援模式恢复已知快照；不要假定等待会恢复 `/etc/nftables.conf`、自有规则文件或状态文件。
...
应用、停用或选择性恢复 Fail2ban 后，先保持当前 SSH 会话，从另一终端验证公钥登录及 `fail2ban status`。这些操作会立即生效，不输出回滚令牌；若异常，请使用已知快照或带外控制台恢复。

只恢复 VPS Guard 自有 jail，而不覆盖第三方 jail：

```bash
sudo vps-guard fail2ban restore <已知快照ID>
```
