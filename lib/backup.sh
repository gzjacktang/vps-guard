...
delete_snapshot() {
	local snapshot_id="$1" confirmed="${2:-0}"
	local snapshot_dir answer
	snapshot_dir="$(snapshot_directory "$snapshot_id")" || return $?
	...
	if [[ "$DRY_RUN" -eq 1 ]]; then
		printf 'dry-run：将删除快照 %s\n' "$snapshot_id"
		return 0
	fi
	if [[ "$confirmed" -ne 1 ]]; then
		printf '确认删除快照 %s？[y/N] ' "$snapshot_id"
		IFS= read -r answer
		case "$answer" in
		y | Y | yes | YES) ;;
		*)
	...
	rm -rf "$snapshot_dir" || {
		error "删除快照失败：$snapshot_id"
		return "$EXIT_FAILURE"
	}
	audit_event snapshot.delete success "snapshot=$snapshot_id"
	printf '已删除快照：%s\n' "$snapshot_id"
}
...
backup_cli() {
	...
	delete)
		[[ "$#" -ge 2 && "$#" -le 3 ]] || {
			error "用法：vps-guard backup delete <快照ID> [--yes]"
			return "$EXIT_USAGE"
		}
		if [[ "$#" -eq 3 && "$3" != "--yes" ]]; then
			error "backup delete 未知参数：$3"
			return "$EXIT_USAGE"
		fi
		delete_snapshot "$2" "$([[ "${3:-}" == "--yes" ]] && printf 1 || printf 0)"
		;;
	retention)
		if [[ "$#" -eq 1 ]]; then
			printf '快照保留数量：%s\n' "$(snapshot_retention)"
		...
		;;
	*)
		error "用法：vps-guard backup <create|list|diff|restore|delete|retention>"
		return "$EXIT_USAGE"
		;;
	esac
}
