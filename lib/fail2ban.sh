...
apply_fail2ban_policy_unlocked() {
	...
	local values current_ip="" ignoreip ports candidate live_stage="" answer snapshot_output snapshot failure_status
	...
	if ! live_stage="$(stage_fail2ban_candidate "$candidate")" || ! mv "$live_stage" "$(fail2ban_config_path)" || ! reload_fail2ban_runtime; then
		rm -f "$live_stage"
		rm -f "$candidate"
		recover_failed_fail2ban_change "$snapshot" || true
...
disable_fail2ban_unlocked() {
	local rollback_minutes="$1" confirmed="$2" answer snapshot_output snapshot
...
restore_fail2ban_from_snapshot_unlocked() {
	local target="$1" minutes="$2" confirmed="$3" target_dir saved candidate="" live_stage="" answer current_output current
	...
	if { [[ -n "$candidate" ]] && ! live_stage="$(stage_fail2ban_candidate "$candidate")"; } ||
		...
		recover_failed_fail2ban_change "$current" || true
		audit_event fail2ban.restore failure "snapshot=$target"
		error "Fail2ban 恢复失败，已尝试恢复操作前状态"
		return "$EXIT_FAILURE"
	fi
	rm -f "$candidate"
	audit_event fail2ban.restore success "snapshot=$target"
...
fail2ban_cli() {
	...
	restore)
		[[ "$#" -ge 2 ]] || {
			error "用法：vps-guard fail2ban restore <快照ID> [--yes]"
			return "$EXIT_USAGE"
		}
		snapshot="$2"
		shift 2
