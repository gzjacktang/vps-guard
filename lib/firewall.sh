...
ensure_firewall_scope_owned_or_free() {
	...
	if [[ -e "$state_path" ]]; then
		...
		error "发现无效的 nftables 防火墙状态文件：$state_path"
...
ensure_no_pending_firewall_rollback() {
	...
	for state_file in "$root"/*/state; do
		...
		case "$hook" in
		ssh-firewall | ssh-restore | ssh-hardening) ;;
		*) continue ;;
		esac
		...
		pending | running)
			...
			error "仍有等待确认的 SSH 自动回滚：$token"
			error "请从新的已验证 SSH 会话确认，或等待其完成后再执行 SSH 操作"
			return "$EXIT_CONFLICT"
			;;
		esac
	done
...
disable_firewall_unlocked() {
	...
	rm -f "$cleanup" || true
	audit_event firewall.disable success "snapshot=$snapshot_id"
	printf 'nftables 防火墙已停用。所有端口将由其他防火墙和上游网络策略决定。\n'
}
...
enable_firewall_unlocked() {
	...
	local tcp_ports udp_ports ssh_ports candidate snapshot_output snapshot_id note
	...
	audit_event "firewall.$operation" success "snapshot=$snapshot_id"
	case "$operation" in
	enable) printf 'nftables 防火墙已启用。' ;;
	...
	enable)
		...
		while [[ "$#" -gt 0 ]]; do
			...
			--udp)
				...
				;;
			--rollback-minutes)
				error "自动回滚仅支持 SSH 管理；nftables 防火墙不支持 --rollback-minutes"
				return "$EXIT_USAGE"
				;;
			--yes)
				confirmed=1
